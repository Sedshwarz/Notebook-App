...
var listem = document.getElementsByClassName("fa fa-list")[0];
var yeni = document.getElementById("yeni");
var yeni2 = document.getElementById("yeni2");
var baslik = document.getElementById("headline");
var geri = document.getElementById("geri");
var yazi = document.getElementById("txt");
...
function enteer(event){if(event.keyCode==13){ sonaOdakla();}}                   // başlıktayken enter basıldığında yazı kısmına geçme

function odakla(){setTimeout(function() { yazi.focus();}, 0);}                  // yazıya focus için hazır fonksiyon
...
function yaziD(event){
  if((event.keyCode==8) && (yazi.innerText.trim()=="")){
    yazi.innerText=""; odakla(); }}                                             // içerik silindiğinde tekrar placeholder göstersin
...
document.addEventListener("keydown",function(){
  ...
  if((yazi.getAttribute("name")===gunc.getAttribute("name")) && (baslik.value==gunc.alt)){
  ...
  }else {kaydet.style.color="#b33939";}                                         // kaydet butonunun renk değişimi olayı
}
else{kaydet.style.color="#b33939";}
});
	...
	if (baslik.value.trim() === ""){
		...
		if (yazi.classList.contains("kayitli")==false){
			...
			var bas = baslik.value; var text = yazi.innerText;
      ...
      li.classList.add("listede");                                              // ilk kaydettiğimizde kayıtlı olacak ki bir daha kaydet dediğimizde üzerine yazsın
          ...
          if(this.parentNode.parentNode.classList.contains("listede")){yazi.classList.remove("kayitli"); kaydet.style.color="#b33939";}
		...
		function notsayi(){																													// hazır fonksiyon oluşturduk, not sayısını ve özelliğini güncellemek için
			...
			else {document.getElementById("qntt").style.height="6vh";}
    ...
    kaydet.style.color="#4b4b4b";
        ...
        for(var z=0;z<list.childElementCount;z++){
          if(list.childNodes[z].classList.contains("listede")){
            ...
            var textGuncel = yazi.innerText;  var basGuncel = baslik.value;
